...
trait MetricsClientFixture extends MockFactory {
	...
	lazy val gauge = mockFunction[String, Double, TagMap, Double, Unit]
	...
	lazy val metrics = new MetricsClient[Unit] {
		...
		def gauge(metric : String, value : Double, tags : TagMap, sampleRate : Double) : Unit = MetricsClientFixture.this.gauge(metric, value, tags, sampleRate)
